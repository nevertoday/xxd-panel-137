...
# XXD Panel 137｜كولاج الحفر والملمس
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-137`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-137 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-137-prompt.en.md) أو [بالصينية](references/xxd-panel-137-prompt.zh-CN.md).
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 137 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
