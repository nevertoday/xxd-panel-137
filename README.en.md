...
# XXD Panel 137｜Etching Texture Collage
...
- `design-only`: the full canvas contains only Panel 137's designed translation; the photograph remains a non-visible reference.
...
Restart the agent session after installation, then invoke `$xxd-panel-137`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-137 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
...
/xxd-panel-137 ./photos --mode design-only --size auto,3:4 --text prompt --locale ja-JP
```

See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-137-prompt.en.md) or [Chinese](references/xxd-panel-137-prompt.zh-CN.md) runtime adapter.
